...
export default function ToolpathPlot() {
  ...
  useEffect(() => {
    const storedEmail = localStorage.getItem("userEmail")?.replace("@", "") || "";
  ...
  const handleChange = async (type: string) => {
    ...
    try {
      const res = await axios.get(`${API}/${route}?email=${encodeURIComponent(email)}`);
        ...
        {[{ src: "pont.html", label: "contour" }, { src: "spont.html", label: "spiral" }].map((item, i) => (
          ...
          >
            <div className="w-full h-[500px] bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
              <iframe
                src={`${API}/static/${encodeURIComponent(email)}/${item.src}`}
                title={`Toolpath Plot ${i + 1}`}
                className="w-full h-full border-0"
              />
              <h1 className="text-shadow-zinc-100">{item.src}</h1>
            </div>

            <motion.button
              className="mt-4 group flex items-center gap-2 px-6 py-3 rounded-2xl bg-white/10 backdrop-blur-md shadow-lg border border-white/20 hover:scale-105 hover:-translate-y-1 transition-all duration-300"
              whileHover={{ scale: 1.05 }}
